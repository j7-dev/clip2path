...
> Press `Ctrl + Alt + V` → the clipboard image is saved as a PNG file, and its **file path is typed at your cursor** in a blink (~0.2s). Your clipboard stays untouched — the image can still be pasted normally.
...
- **AutoHotkey v2** registers the global hotkey and types the path as direct Unicode input — no clipboard involved, immune to IME interception, and reliable across consoles, classic controls and Electron apps.
...
3. Press `Ctrl + Alt + V`
4. The PNG path appears at your cursor; a toast notification confirms the save
