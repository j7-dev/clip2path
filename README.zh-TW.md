...
> 按下 `Ctrl + Alt + V` → 剪貼簿圖片自動存成 PNG，並**在游標處一次貼上檔案路徑**。剪貼簿會自動備份還原——貼完路徑後，圖片照樣可以正常貼上。
...
- **AutoHotkey v2** 註冊全域熱鍵，貼上路徑後自動還原原本的剪貼簿內容（`ClipboardAll` 完整備份）
